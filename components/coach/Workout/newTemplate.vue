    ...
    <v-card class="fit-dailog-card" >
        ...
        <v-card-text style="padding:0">
            <v-row style="margin:0">
                    ...
                    <div class="firstHalf">
                        ...
                        <div style="padding:0;" class="exercsie-block">
                            ...
                            <div v-if="filterdList == null">
                                <draggable class="list-group exercises_list" :group="{ name: 'my-group2', pull: 'clone'}" :list="exercises" :animation="200" :sort="false" ghostClass="moving-card">
                                    <div class="list-group-item" v-for="(element ) in exercises" :key="element.id">
                                        <div class="exercise">
                            ...
                            <div v-else>
                                <draggable class="list-group" :list="exercises" group="people" @change="log">
                                    ...
                                    <div class="list-group-item" v-for="(element ) in filterdList" :key="element.id">
                                        <div class="exercise">
                                            <div class="image-title">
                                                <img v-if="element.images != null && element.images.length > 0" :src=element.images[0] style="border-radius: 10%;" alt="">
                                                <div class="text"> {{ element.title }}
                ...
                <v-col cols="12" md="8" sm="12" style="padding:0;">
                    <div class="secondHalf">
                        <v-form ref="myform" lazy-validation class="form">
                            <v-text-field dense v-model="workout_name" label="Name of Workout" clearable placeholder="Name of Exercise" outlined :rules="[v => !!v || 'This filed is required']"></v-text-field>
                            <v-textarea dense outlined v-model="description" label="Description " placeholder="add your description" rows="1" row-height="20"></v-textarea>

                            <draggable class="list-group" :list="single_exercises" group="my-group2" @change="log">
                                <div :class="['dragg']" v-if="single_exercises.length==0 && ActiveSection == 'drag-drop'">
                                ...
                                <div style="margin-bottom: 10px;" class="list-group-item my-handle" v-for="(exercise,index) in single_exercises" :key="exercise.id">
                                    <exerciseinsection @updateEx="updateEx" @deleteExercise="deleteExerciseSingle(exercise)" :exercise="exercise" :Ident="index"></exerciseinsection>
                                </div>
                            </draggable>
                            ...
                            <div class="sections-drag" v-for="(new_section,index) in newSections" :key="new_section.id">
                                <div class="section-header d-flex align-items-center">
                                    <v-icon color="#fff" class="mr-4" @click="hideSection(new_section)"> {{ new_section.hide ? 'mdi-menu-up' : 'mdi-menu-down'}}</v-icon>
                                ...
                                <div class="section-body" v-show="!new_section.hide">
                                    ...
                                    <draggable class="list-group" :list="new_section.exercises" group="my-group2" @change="log">
                                        ...
                                        <div style="margin-bottom: 10px;" class="list-group-item my-handle" v-for="(exercise,index2) in new_section.exercises.slice().reverse()" :key="exercise.id">
                                            <exerciseinsection @updateEx="updateExSection" @deleteExercise="deleteExercise(new_section,exercise)" :sectionindex="index" :exercise="exercise" :Ident="index2"></exerciseinsection>
                                        </div>
                                    </draggable>
                                </div>
                            </div>
        ...
        <v-card-actions class="d-flex justify-end">
            <v-btn class="mt-4 mb-4 mr-4 fit-btn" outlined color="primary" @click="$emit('close')">
                Cancel
            </v-btn>
            <v-btn class="mt-4 mb-4  fit-btn" color="primary" @click="CreateWorkout">
                Create
            </v-btn>
   ...
   watch:{
    ...
    methods: {
        ...
        async CreateWorkout() {
            ...
            //    this.single_exercises.forEach(function (exercise) {
            //         if (exercise.title && exercise.title != null) {
            //             exercise.exercise_id = exercise.id;
            //             exercise.newEx = true;
            //         }
            //     })
            //     for (var i = 0; i < this.newSections.length; i++) {
            //         this.newSections[i].exercises.forEach(function (exercise) {
            //             if (exercise.title && exercise.title != null) {
            //                 exercise.exercise_id = exercise.id;
            //                 exercise.newEx = true;
            //             }
            ...
            //     }
                ...
                var Data = {
                    title: this.workout_name,
                    description: this.description,
                    single_exercises: this.single_exercises,
                ...
                /* ======= Send To API  ======= */
                await this.$axios.post('/workouts/add', Data).then((response) => {
                    myloader.close();
        ...
        addSet(section, exercise) {
            var mysectionIndex = this.newSections.indexOf(section)
            var myExIndex = this.newSections[mysectionIndex].exercises.indexOf(exercise)
            this.newSections[mysectionIndex].exercises[myExIndex].sets.push({})
        },
        deleteItem(item) {
        ...
        deleteSection(section) {
            this.newSections.splice(this.newSections.indexOf(section), 1)
        },
        ...
        deleteExercise(section, exercise) {
            var mysectionIndex = this.newSections.indexOf(section)
            var myExIndex = this.newSections[mysectionIndex].exercises.indexOf(exercise)
            this.newSections[mysectionIndex].exercises.splice(myExIndex, 1)
        },
        ...
        updateEx(obj) {
            ...
            this.single_exercises[obj.index] = obj
        },
        updateExSection(obj) {
            this.newSections[obj.section_index].exercises[obj.index] = obj
        },
        searchfilter() {
