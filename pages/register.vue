<template>
<v-row style="height:100vh; overflow:hidden;margin:0">
    <v-col cols="12" md="7" sm="12" style="padding:0;" class="d-md-block d-none">
        <div class="auth_bg"> </div>
    </v-col>
    <v-col cols="12" md="5" sm="12" class="scroll-style" style="padding:0;height:100%;overflow-y: auto">
        <div class="auth-steps">
            <div class="logo">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 149.03 175.73">
                    <defs>

                        <style>
                            .cls-1 {
                                fill: #34be82;
                            }
                        </style>
                    </defs>
                    <title>Asset 2</title>
                    <g id="Layer_2" data-name="Layer 2">
                        <g id="Layer_1-2" data-name="Layer 1">
                            <path class="cls-1" d="M121,112.48c-11.34-7.55-24-12.12-36.95-16-39.18-11-63.55-1.92-63.55-1.92C32.56,99.31,74.64,113,93.33,124.94c-.07,0-48-15.6-75.4-17.72a62.88,62.88,0,0,0,5.68,29.95,64,64,0,0,0,22.61,27.07c15.25,10.22,32,13.69,50.08,10.15,1.47-.29,2.93-.65,4.39-1,.1,0,.18-.13.27-.19C99.72,169.08,90.42,156,70.5,146.33,84.22,149,101.59,160,111.76,169c16.47-7.36,23.7-18.58,25.36-22.23,2.71-5.94,2.44-11.78-.35-17.65-3.47-7.27-9.26-12.33-15.76-16.66" />
                            <path class="cls-1" d="M143,83.81a88.27,88.27,0,0,0-7.37-12.31,76.68,76.68,0,0,0-6.89-8,1.64,1.64,0,0,1-.52-1.64,3.4,3.4,0,0,1,.29-.8c3.78-8,6.36-16.23,6-25.16a28,28,0,0,0-4.71-15c-7.06-10.48-17.2-16.13-29.18-19a73,73,0,0,0-33.43-.06c-10,2.31-18.88,6.83-25.71,14.78C35.41,23.77,33.34,32,34.33,41.18A59.17,59.17,0,0,0,39.52,59.6c.17.37.31.66.43.9a5.92,5.92,0,0,0,.48.83s22.15,1.36,35.08,5.36a72.87,72.87,0,0,1,16.81,6.39,242.26,242.26,0,0,0-62.06-2.52A259.72,259.72,0,0,0,0,75c0,.22,0,.43,0,.64l10.32,0c5.37,0,10.73.14,16.08.37a317.92,317.92,0,0,1,47.21,5.32A187.77,187.77,0,0,1,115.71,94.3a95.22,95.22,0,0,1,21,12.6c1.51,1.23,3,2.51,4.37,3.87a87.82,87.82,0,0,1,7.24,8.48c2.45-12.6-2.23-28.77-5.36-35.44M83.3,44.65a67,67,0,0,0-27.49,5.86c-2-5.55-3.63-11.69-2.26-17.46C55,26,64,22.23,69.88,20.63A54.19,54.19,0,0,1,91,19.33c8.37,1,16.09,3.52,22,10.07a10.27,10.27,0,0,1,2.75,6c.44,4.84-.82,9.38-2.59,13.8a14,14,0,0,1-1,1.89A66.93,66.93,0,0,0,83.3,44.65" />
                        </g>
                    </g>
                </svg>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 367 77.7" style="margin:0 20px">
                    <title>Asset 3</title>
                    <g id="Layer_2" data-name="Layer 2">
                        <g id="Layer_1-2" data-name="Layer 1">
                            <path d="M72.35,10.9h10.9V0H72.35Zm0,66.8h10.9V19.17H72.35Z" />
                            <polygon points="350.77 62.5 367 19.17 355.39 19.17 344.31 48.42 350.77 62.5" />
                            <polygon points="340.56 58.02 322.7 19.17 310.29 19.17 293.44 58.02 277.59 19.17 265.99 19.17 289 77.7 297.77 77.7 297.67 77.6 297.88 77.7 316.44 34.21 336.12 77.7 345 77.7 347.2 71.85 340.56 58.02" />
                            <path d="M128.36,30.07V19.17h-24V0H93.44V64.58A13.1,13.1,0,0,0,106.56,77.7h21.8V66.8h-21.8a2.24,2.24,0,0,1-2.22-2.22V30.07Z" />
                            <path d="M181.33,19.17H134.91V77.7h10.9V32.29A2.24,2.24,0,0,1,148,30.07h33.3a2.24,2.24,0,0,1,2.22,2.22V77.7h10.9V32.29a13.09,13.09,0,0,0-13.12-13.12" />
                            <path d="M250.25,19.17H217a13.1,13.1,0,0,0-13.12,13.12V64.58A13.1,13.1,0,0,0,217,77.7h33.3a13.1,13.1,0,0,0,13.12-13.12V32.29a13.09,13.09,0,0,0-13.12-13.12m2.22,45.41a2.24,2.24,0,0,1-2.22,2.22H217a2.24,2.24,0,0,1-2.22-2.22V32.29A2.24,2.24,0,0,1,217,30.07h33.3a2.24,2.24,0,0,1,2.22,2.22Z" />
                            <rect x="17.92" y="35.92" width="37.78" height="10.9" />
                            <polygon points="10.9 35.92 10.9 15.94 66.6 15.94 66.6 5.04 0 5.04 0 77.7 10.9 77.7 10.9 46.82 10.9 35.92" />
                        </g>
                    </g>
                </svg>
            </div>
            <div class="steps-container">
                <step1 v-if="step==0" @next="next"></step1>
                <step2 v-if="step==1" @next="next" :error_msg="error_msg" :type="registerData.type"></step2>
                <verifyComp v-if="verifyemail" :email="registerData.data.email"></verifyComp>
            </div>

        </div>

    </v-col>
</v-row>
</template>

<script>
import step1 from '../components/global/register/step-1.vue'
import step2 from '../components/global/register/step-2.vue'
import verifyComp from '../components/global/register/verify.vue'
import * as firestoreService from '@/plugins/chat-database/firestore'
import * as firebaseService from '@/plugins/chat-database/firebase'
import * as storageService from '@/plugins/chat-database/storage'
export default {
    auth: false,
    components: {
        step1,
        step2,
        verifyComp
    },
    data() {
        return {
            step: 0,
            registerData: {},
            error_msg: "",
            verifyemail: false,
            mytoken: ""
        }
    },
    methods: {
        next(data) {
            if (Object.keys(data)[0] == 'type') {
                this.registerData.type = data.type;
            }
            if (this.step == 0) {
                this.step += 1;
            } else {
                this.registerData.data = data.data;
                this.Register(this.registerData);
            }

        },
        async Register(registerData) {
            const myloader = this.myloading = this.$vs.loading({
                color: '#34BE82'
            });
            var mypage = this;
            mypage.error_msg = "";
            const mydate = registerData.data;
            mydate.type = registerData.type;
            await this.$axios.post('/register', mydate).then((response) => {
                this.step = 3;
                this.verifyemail = true;
                this.mytoken = response.data.access_token;
                this.$auth.strategy.token.set(response.data.access_token)
                this.startChat(response.data.user);
                myloader.close();

            }).catch(function (error) {
                myloader.close();
                console.log(error.response)
                if (error.response.data.errors.length > 0) {
                    mypage.error_msg = error.response.data.errors[0];
                } else {
                    mypage.error_msg = error.response.data.message;
                }

            });
        },
        async CreateRoom2(user1, user2) {
            await firestoreService.addRoom({
                users: [user1._id, user2._id],
                lastUpdated: new Date()
            })
        },
        async startChat(user) {
            const myloader = this.$vs.loading({
                target: this.$refs.mycontainer,
                color: '#34BE82'
            })
            const user1 = {
                _id: "999999",
                username: "FitNow",
                avatar:"https://i.ibb.co/ZX3WF8b/FitNow.png",
            }
            await firestoreService.addIdentifiedUser(user1._id, user1)

            const user2 = {
                _id: user.id.toString(),
                username: user.fullname,
                avatar: user.photo != null ? user.photo : 'https://www.gravatar.com/avatar/00000000000000000000000000000000?d=mp&f=y',
            }

            await firestoreService.addIdentifiedUser(user2._id, user2)
            await this.CreateRoom2(user1, user2)
            myloader.close()

        },
    }
}
</script>

<style lang="scss" scoped>
.v-application p {
    margin: 0 !important;
    padding: 0 !important;
}

.auth_div {
    overflow-y: auto;
    height: 100vh;
}

.auth_bg {
    background-image: url('~/assets/img/login_bg.png');
    height: 100vh;
    background-size: cover;
}

.auth-steps {
    width: 70% !important;
    margin: 4rem auto;
    margin-top: calc(100% - 95%);
    text-align: center;
    @media (max-width:760px) {
        width: 90% !important;
    }
    .steps-container{
    background-color: #fff;
    padding: 2rem 8%;
    border-radius: 10px;
    box-shadow: 0px 0px 20px 20px #f1f1f169;
    }

}

.logo {
    padding: 0.5rem 0;
    margin-bottom: 2rem;
    @media (max-width:760px) {
        margin-bottom: 2rem;
    }

    svg {
        height: 50px;

        @media (max-width:760px) {
            height: 40px;
        }
    }
}
.auth-btn {
    font-size: 18px;
    font-weight: 600;
    padding-top: 1rem;
    padding-bottom: 1rem;
    height: 55px !important;
}
</style>
