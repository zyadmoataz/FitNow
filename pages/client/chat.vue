...
import {
    parseTimestamp,
    formatTimestamp
} from '@/plugins/chat-database/dates'
...
export default {
          ...
          breadcrumbPath: [{
           ...
           myuser:null,
            ...
            styles: {
				...
				general: {
			color: '#000',
			colorButtonClear: '#4cc691',
			colorButton: '#fff',
			backgroundColorButton: '#4cc691',
			backgroundInput: '#fff',
			...
			sendDisabled: '#646a70',
			emoji: '#4cc691',
			emojiReaction: '#4cc691',
			document: '#1976d2',
			pencil: '#4cc691',
			checkmark: '#fff',
			checkmarkSeen: '#f0d90a',
			eye: '#4cc691',
			dropdownMessage: '#4cc691',
    ...
    async mounted() {
        ...
        if (this.token) {
            ...
            await this.$axios.get('/trainee/data').then((response) => {
                this.myuser = response.data.data.user;
        ...
        this.currentUserId = this.myuser.id.toString();
        await this.fetchRooms();
        firebaseService.updateUserOnlineStatus(this.currentUserId)
		if(this.$route.params.id){
			this.listenLastMessage(this.$route.params.id)
		}
        myloader.close();
