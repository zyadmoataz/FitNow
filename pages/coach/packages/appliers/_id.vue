...
<div>
    ...
    <v-card class="fit-card" v-if="myPackage != null">
        <v-card-title class="fit-card-title mb-2">
            {{myPackage.title}} - Appliers
        </v-card-title>
        <v-row>
            <v-col>
                <v-data-table sort-desc :headers="headers" :items="myPackage.appliers" sort-by="id" class="elevation-1">
                    <template v-slot:[`item.actions`]="{ item }">
                        <span @click="$router.push('/coach/packages/appliers/view/'+item.id)" style="cursor: pointer;">
